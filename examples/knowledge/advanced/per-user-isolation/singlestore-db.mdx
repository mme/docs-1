---
title: "Per-User Isolation: SingleStore"
description: "Each user gets a private view of one shared knowledge base."
source: cookbook/07_knowledge/04_advanced/07_per_user_isolation/singlestore_db.py
---

Each user gets a private view of one shared knowledge base. Documents uploaded with a user_id are visible only to that user; documents uploaded without one are shared with everyone, and an admin (user_id=None) sees all of it.
...
<Steps>
  ...
  <Step title="Run the example">
    Save the code above as `singlestore_db.py`, then run:
    ```bash
    python singlestore_db.py
    ```
  </Step>
...
Full source: [cookbook/07_knowledge/04_advanced/07_per_user_isolation/singlestore_db.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/07_knowledge/04_advanced/07_per_user_isolation/singlestore_db.py)
