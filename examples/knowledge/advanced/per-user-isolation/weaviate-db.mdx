---
title: "Per-User Isolation: Weaviate"
description: "Each user gets a private view of one shared knowledge base."
source: cookbook/07_knowledge/04_advanced/07_per_user_isolation/weaviate_db.py
---

Each user gets a private view of one shared knowledge base. Documents inserted with a user_id are visible only to that user, documents inserted without one are shared with everyone.
...
<Steps>
  ...
  <Step title="Install dependencies">
    ```bash
    uv pip install -U agno openai weaviate weaviate-client
    ```
  </Step>
  ...
  <Step title="Export your API keys">
    ...
    export WCD_API_KEY="your_wcd_api_key_here"
    ```

    ```bash Windows
    $Env:OPENAI_API_KEY="your_openai_api_key_here"
    $Env:WCD_API_KEY="your_wcd_api_key_here"
    ```
    </CodeGroup>
  </Step>

  <Step title="Run the example">
    Save the code above as `weaviate_db.py`, then run:
    ```bash
    python weaviate_db.py
    ```
  </Step>
...
Full source: [cookbook/07_knowledge/04_advanced/07_per_user_isolation/weaviate_db.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/07_knowledge/04_advanced/07_per_user_isolation/weaviate_db.py)
