---
title: "Per-User Isolation: MongoDB"
description: "Each user gets a private view of one shared knowledge base."
source: cookbook/07_knowledge/04_advanced/07_per_user_isolation/mongo_db.py
---

Each user gets a private view of one shared knowledge base. Documents uploaded with a user_id are visible only to that user; documents uploaded without one are shared with everyone. Searching with user_id=None is the admin view and sees the whole corpus.
...
<Steps>
  ...
  <Step title="Install dependencies">
    ```bash
    uv pip install -U agno bson openai pymongo
    ```
  </Step>
  ...
  <Step title="Run the example">
    Save the code above as `mongo_db.py`, then run:
    ```bash
    python mongo_db.py
    ```
  </Step>
...
Full source: [cookbook/07_knowledge/04_advanced/07_per_user_isolation/mongo_db.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/07_knowledge/04_advanced/07_per_user_isolation/mongo_db.py)
