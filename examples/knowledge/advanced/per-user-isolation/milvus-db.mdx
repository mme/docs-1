---
title: "Per-User Isolation: Milvus"
description: "Each user gets a private view of one shared knowledge base."
source: cookbook/07_knowledge/04_advanced/07_per_user_isolation/milvus_db.py
---

Each user gets a private view of one shared knowledge base. Documents inserted with a user_id belong to that user; documents inserted without one are shared.
...
<Steps>
  ...
  <Step title="Install dependencies">
    ```bash
    uv pip install -U agno numpy openai pymilvus
    ```
  </Step>
  ...
  <Step title="Run the example">
    Save the code above as `milvus_db.py`, then run:
    ```bash
    python milvus_db.py
    ```
  </Step>
...
Full source: [cookbook/07_knowledge/04_advanced/07_per_user_isolation/milvus_db.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/07_knowledge/04_advanced/07_per_user_isolation/milvus_db.py)
