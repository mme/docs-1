---
title: "Per-User Isolation: Upstash"
description: "Each user gets a private view of one shared knowledge base."
source: cookbook/07_knowledge/04_advanced/07_per_user_isolation/upstash_db.py
---

Each user gets a private view of one shared knowledge base. Documents uploaded with a user_id are visible only to that user; documents uploaded without one are shared with everyone.
...
<Steps>
  ...
  <Step title="Install dependencies">
    ```bash
    uv pip install -U agno openai upstash-vector
    ```
  </Step>
  ...
  <Step title="Export environment variables">
    ...
    export UPSTASH_VECTOR_REST_TOKEN="your_upstash_vector_rest_token_here"
    export UPSTASH_VECTOR_REST_URL="your_upstash_vector_rest_url_here"
    ```

    ```bash Windows
    $Env:OPENAI_API_KEY="your_openai_api_key_here"
    $Env:UPSTASH_VECTOR_REST_TOKEN="your_upstash_vector_rest_token_here"
    $Env:UPSTASH_VECTOR_REST_URL="your_upstash_vector_rest_url_here"
    ```
    </CodeGroup>
  </Step>

  <Step title="Run the example">
    Save the code above as `upstash_db.py`, then run:
    ```bash
    python upstash_db.py
    ```
  </Step>
...
Full source: [cookbook/07_knowledge/04_advanced/07_per_user_isolation/upstash_db.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/07_knowledge/04_advanced/07_per_user_isolation/upstash_db.py)
