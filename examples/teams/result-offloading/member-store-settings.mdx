---
title: "Member Store Settings"
description: "The team's own setting is never written onto a member, and the binding is redone every time the team initializes, so a member moved to another team follows that team."
source: cookbook/03_teams/27_result_offloading/member_store_settings.py
---

```python member_store_settings.py
"""
...
<Steps>
  ...
  <Step title="Run the example">
    Save the code above as `member_store_settings.py`, then run:
    ```bash
    python member_store_settings.py
    ```
  </Step>
...
Full source: [cookbook/03_teams/27_result_offloading/member_store_settings.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/03_teams/27_result_offloading/member_store_settings.py)
