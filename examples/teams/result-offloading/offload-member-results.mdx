---
title: "Offload Member Results"
description: "A member's answer reaches the team leader as the result of the delegation tool, so it is the payload that grows a team session."
source: cookbook/03_teams/27_result_offloading/offload_member_results.py
---

A member's answer reaches the team leader as the result of the delegation tool, so it is the payload that grows a team session. With `offload_tool_results` set, the leader's transcript holds a short envelope with a result id, and the full answer is stored as a file the leader can read back.

```python offload_member_results.py
"""
...
<Steps>
  ...
  <Step title="Run the example">
    Save the code above as `offload_member_results.py`, then run:
    ```bash
    python offload_member_results.py
    ```
  </Step>
...
Full source: [cookbook/03_teams/27_result_offloading/offload_member_results.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/03_teams/27_result_offloading/offload_member_results.py)
