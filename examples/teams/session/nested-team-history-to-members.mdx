---
title: "Nested Team History for Members"
description: "Demonstrates `add_team_history_to_members` for nested teams: when a parent team delegates to a nested sub-team, the sub-team is given ITS OWN prior history (filtered by the sub-team's id) rather than the root team leader's history."
source: cookbook/03_teams/07_session/nested_team_history_to_members.py
---

```python nested_team_history_to_members.py
"""
...
<Steps>
  ...
  <Step title="Run the example">
    Save the code above as `nested_team_history_to_members.py`, then run:
    ```bash
    python nested_team_history_to_members.py
    ```
  </Step>
...
Full source: [cookbook/03_teams/07_session/nested_team_history_to_members.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/03_teams/07_session/nested_team_history_to_members.py)
