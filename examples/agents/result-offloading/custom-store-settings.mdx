...
description: "`offload_tool_results=True` uses the defaults: results longer than 16,000 characters are stored, the envelope previews 20 lines or 1,200 characters, and payloads live until the session is deleted."
source: cookbook/02_agents/22_result_offloading/04_custom_store_settings.py
---

```python custom_store_settings.py
"""
...
<Steps>
  ...
  <Step title="Run the example">
    Save the code above as `custom_store_settings.py`, then run:
    ```bash
    python custom_store_settings.py
    ```
  </Step>
...
Full source: [cookbook/02_agents/22_result_offloading/04_custom_store_settings.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/02_agents/22_result_offloading/04_custom_store_settings.py)
