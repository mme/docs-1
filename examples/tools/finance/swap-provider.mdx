---
title: "Swap the Provider, Keep the Agent"
description: "The point of FinanceTools: the agent is written once against a fixed tool surface, and the data provider is a configuration choice."
source: cookbook/91_tools/finance/03_swap_provider.py
---

```python swap_provider.py
"""
...
<Steps>
  ...
  <Step title="Run the example">
    Save the code above as `swap_provider.py`, then run:
    ```bash
    python swap_provider.py
    ```
  </Step>
...
Full source: [cookbook/91_tools/finance/03_swap_provider.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/91_tools/finance/03_swap_provider.py)
