---
title: "Atomic Mail Workflow"
description: "Runs AtomicMailTools (https://atomicmail.ai) end to end inside a deterministic Agno workflow -- no LLM required."
source: cookbook/91_tools/atomic_mail_tools.py
---

```python atomic_mail_tools.py
"""
...
<Steps>
  ...
  <Step title="Export environment variables">
    <CodeGroup>
    ```bash Mac/Linux
    export ATOMIC_MAIL_TEST_RECIPIENT="your_atomic_mail_test_recipient_here"
    ```

    ```bash Windows
    $Env:ATOMIC_MAIL_TEST_RECIPIENT="your_atomic_mail_test_recipient_here"
    ```
    </CodeGroup>
  </Step>

  <Step title="Run the example">
    Save the code above as `atomic_mail_tools.py`, then run:
    ```bash
    python atomic_mail_tools.py
    ```
  </Step>
...
Full source: [cookbook/91_tools/atomic_mail_tools.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/91_tools/atomic_mail_tools.py)
