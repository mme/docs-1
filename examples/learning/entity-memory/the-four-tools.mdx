---
title: "Entity Memory: The Four Tools"
description: "Entity memory is the agent's knowledge base about the world: the people, projects, companies and systems around the user."
source: cookbook/08_learning/04_entity_memory/01_the_four_tools.py
---

Corrections are just new facts: stating "radar shipped" retires "radar is blocked on review" automatically (fact supersession), and facts render with as-of dates so newer truth outranks older.

```python the_four_tools.py
"""
...
<Steps>
  ...
  <Step title="Run the example">
    Save the code above as `the_four_tools.py`, then run:
    ```bash
    python the_four_tools.py
    ```
  </Step>
...
Full source: [cookbook/08_learning/04_entity_memory/01_the_four_tools.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/08_learning/04_entity_memory/01_the_four_tools.py)
