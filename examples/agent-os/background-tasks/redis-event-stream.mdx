...
description: "AgentOS with Redis-coordinated background runs (multi-container ready)."
source: cookbook/05_agent_os/background_tasks/redis_event_stream.py
---

```python redis_event_stream.py
...
<Steps>
  ...
  <Step title="Run the example">
    Save the code above as `redis_event_stream.py`, then run:
    ```bash
    python redis_event_stream.py
    ```
  </Step>
...
Full source: [cookbook/05_agent_os/background_tasks/redis_event_stream.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/05_agent_os/background_tasks/redis_event_stream.py)
