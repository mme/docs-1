---
title: "JWT authentication with asymmetric keys"
description: "Use an RS256 private key only in the token issuer and give AgentOS only the public verification key."
source: cookbook/05_agent_os/07_security/asymmetric_keys.py
---

Use an RS256 private key only in the token issuer and give AgentOS only the public verification key. A local request proves signature and audience verification before the server starts.

```python asymmetric_keys.py
"""
...
<Steps>
  ...
  <Step title="Install dependencies">
    ```bash
    uv pip install -U "agno[os]" cryptography openai
    ```
  </Step>
  ...
  <Step title="Run the example">
    Save the code above as `asymmetric_keys.py`, then run:
    ```bash
    python asymmetric_keys.py
    ```
  </Step>
...
Full source: [cookbook/05_agent_os/07_security/asymmetric_keys.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/05_agent_os/07_security/asymmetric_keys.py)
