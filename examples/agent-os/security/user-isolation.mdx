---
title: "JWT RBAC with per-user data isolation"
description: "Turn on AuthorizationConfig(user_isolation=True) so non-admin session reads and writes are pinned to the JWT subject."
source: cookbook/05_agent_os/07_security/user_isolation.py
---

Turn on AuthorizationConfig(user_isolation=True) so non-admin session reads and writes are pinned to the JWT subject. The smoke creates two sessions, proves each user sees only their own row, and proves admin bypass.
...
Full source: [cookbook/05_agent_os/07_security/user_isolation.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/05_agent_os/07_security/user_isolation.py)
