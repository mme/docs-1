---
title: "Per-user knowledge ownership"
description: "Turn on AuthorizationConfig(user_isolation=True) so every knowledge content row is owned by the JWT subject, and a row with no owner is shared, org-wide content."
source: cookbook/05_agent_os/07_security/user_isolation_knowledge.py
---
...
```python user_isolation_knowledge.py
"""
...
Full source: [cookbook/05_agent_os/07_security/user_isolation_knowledge.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/05_agent_os/07_security/user_isolation_knowledge.py)
