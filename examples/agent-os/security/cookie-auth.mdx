---
title: "JWT authentication from an HTTP-only cookie"
description: "Mount login/logout routes on the base FastAPI app, read JWTs from a secure cookie, and enforce AgentOS scopes normally."
source: cookbook/05_agent_os/07_security/cookie_auth.py
---

Mount login/logout routes on the base FastAPI app, read JWTs from a secure cookie, and enforce AgentOS scopes normally. The smoke proves both the unauthenticated 401 and authenticated 200 paths.
...
<Steps>
  ...
  <Step title="Run the example">
    Save the code above as `cookie_auth.py`, then run:
    ```bash
    python cookie_auth.py
    ```
  </Step>
...
Full source: [cookbook/05_agent_os/07_security/cookie_auth.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/05_agent_os/07_security/cookie_auth.py)
