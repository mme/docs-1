---
title: "Serve one AgentOS gateway over local and remote components"
description: "The gateway registers a local Agent alongside native AgentOS, Agno A2A REST, and Google ADK JSON-RPC components."
source: cookbook/05_agent_os/20_remote/05_gateway.py
---

The gateway registers a local Agent alongside native AgentOS, Agno A2A REST, and Google ADK JSON-RPC components. Callers use one AgentOS API on port 7777 without needing to know which transport backs each entity.
...
<Steps>
  ...
  <Step title="Run the example">
    Save the code above as `gateway.py`, then run:
    ```bash
    python gateway.py
    ```
  </Step>
...
Full source: [cookbook/05_agent_os/20_remote/05_gateway.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/05_agent_os/20_remote/05_gateway.py)
