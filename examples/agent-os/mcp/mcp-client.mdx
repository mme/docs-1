---
title: "Drive the AgentOS MCP run lifecycle"
description: "Use a protocol-level FastMCP client to discover the eight built-in tools, pause and continue one agent run, cancel a second paused run, and read the continued session back."
source: cookbook/05_agent_os/14_mcp/mcp_client.py
---
...
<Steps>
  ...
  <Step title="Run the example">
    Save the code above as `mcp_client.py`, then run:
    ```bash
    python mcp_client.py
    ```
  </Step>
...
Full source: [cookbook/05_agent_os/14_mcp/mcp_client.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/05_agent_os/14_mcp/mcp_client.py)
