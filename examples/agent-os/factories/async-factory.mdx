---
title: "Use Sync and Async Factories Together"
description: "Register one synchronous and one asynchronous AgentFactory on the same AgentOS and call both through the same async-aware REST resolver."
source: cookbook/05_agent_os/21_factories/07_async_factory.py
---

```python async_factory.py
"""
...
<Steps>
  ...
  <Step title="Run the example">
    Save the code above as `async_factory.py`, then run:
    ```bash
    python async_factory.py
    ```
  </Step>
...
Full source: [cookbook/05_agent_os/21_factories/07_async_factory.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/05_agent_os/21_factories/07_async_factory.py)
