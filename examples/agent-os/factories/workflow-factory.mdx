---
title: "Build a Workflow per Request"
description: "Register a WorkflowFactory that creates a fresh tenant-aware draft-and-edit pipeline for every Workflow run."
source: cookbook/05_agent_os/21_factories/06_workflow_factory.py
---

```python workflow_factory.py
"""
...
<Steps>
  ...
  <Step title="Run the example">
    Save the code above as `workflow_factory.py`, then run:
    ```bash
    python workflow_factory.py
    ```
  </Step>
...
Full source: [cookbook/05_agent_os/21_factories/06_workflow_factory.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/05_agent_os/21_factories/06_workflow_factory.py)
