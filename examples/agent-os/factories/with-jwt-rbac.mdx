---
title: "Grant Factory Tools from Trusted JWT Claims"
description: "Grant factory-agent tools from verified JWT role claims and return 403 for unsupported roles."
source: cookbook/05_agent_os/21_factories/03_with_jwt_rbac.py
---

Use verified JWT claims to choose an Agent's tools per request, keep client input out of authorization decisions, and map FactoryPermissionError to 403.

```python with_jwt_rbac.py
"""
...
<Steps>
  ...
  <Step title="Run the example">
    Save the code above as `with_jwt_rbac.py`, then run:
    ```bash
    python with_jwt_rbac.py
    ```
  </Step>
...
Full source: [cookbook/05_agent_os/21_factories/03_with_jwt_rbac.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/05_agent_os/21_factories/03_with_jwt_rbac.py)
