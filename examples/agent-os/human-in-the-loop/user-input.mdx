---
title: "Resolve multi-round user input for an AgentOS team"
description: "A member agent collects a traveller name and trip preferences in two separate tool calls."
source: cookbook/05_agent_os/05_human_in_the_loop/user_input.py
---

A member agent collects a traveller name and trip preferences in two separate tool calls. Each ``requires_user_input`` call pauses the team run. The client fills the returned requirement schema and calls the team ``continue`` route until the run completes.
...
Full source: [cookbook/05_agent_os/05_human_in_the_loop/user_input.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/05_agent_os/05_human_in_the_loop/user_input.py)
