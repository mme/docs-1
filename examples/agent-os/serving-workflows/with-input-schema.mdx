---
title: "Expose a Workflow input schema"
description: "Define a Pydantic input_schema on a served workflow, then use --demo to fetch GET /workflows/{id} and inspect the JSON schema AgentOS exposes to clients."
source: cookbook/05_agent_os/09_serving_workflows/with_input_schema.py
---

```python with_input_schema.py
"""
...
<Steps>
  ...
  <Step title="Run the example">
    Save the code above as `with_input_schema.py`, then run:
    ```bash
    python with_input_schema.py
    ```
  </Step>
...
Full source: [cookbook/05_agent_os/09_serving_workflows/with_input_schema.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/05_agent_os/09_serving_workflows/with_input_schema.py)
