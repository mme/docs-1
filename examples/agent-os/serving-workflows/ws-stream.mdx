---
title: "Stream a served Workflow over WebSocket"
description: "Connect to the workflow-only /workflows/ws surface, send start-workflow, and consume indexed workflow events until the served workflow completes."
source: cookbook/05_agent_os/09_serving_workflows/ws_stream.py
---
...
<Steps>
  ...
  <Step title="Install dependencies">
    ```bash
    uv pip install -U websockets
    ```
  </Step>

  <Step title="Run the example">
    Save the code above as `ws_stream.py`, then run:
    ```bash
    python ws_stream.py
    ```
  </Step>
...
Full source: [cookbook/05_agent_os/09_serving_workflows/ws_stream.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/05_agent_os/09_serving_workflows/ws_stream.py)
