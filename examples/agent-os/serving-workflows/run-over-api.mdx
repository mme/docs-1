---
title: "Run Over API"
description: "Call basic.py through raw HTTP: create a non-streaming run, consume an SSE event stream, and list the persisted runs for the shared workflow session."
source: cookbook/05_agent_os/09_serving_workflows/run_over_api.py
---

```python run_over_api.py
"""
...
<Steps>
  ...
  <Step title="Run the example">
    Save the code above as `run_over_api.py`, then run:
    ```bash
    python run_over_api.py
    ```
  </Step>
...
Full source: [cookbook/05_agent_os/09_serving_workflows/run_over_api.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/05_agent_os/09_serving_workflows/run_over_api.py)
