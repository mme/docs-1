---
title: "List and continue from AgentOS run checkpoints"
description: "Create a run with ``checkpoint=\"tool-batch\"``, list its persisted continuation boundaries over HTTP, then continue from a selected ``message_index``."
source: cookbook/05_agent_os/04_run_lifecycle/checkpoints.py
---
...
<Steps>
  ...
  <Step title="Run the example">
    Save the code above as `checkpoints.py`, then run:
    ```bash
    python checkpoints.py
    ```
  </Step>
...
Full source: [cookbook/05_agent_os/04_run_lifecycle/checkpoints.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/05_agent_os/04_run_lifecycle/checkpoints.py)
