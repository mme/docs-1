...
description: "Submit a non-streaming background run, observe the HTTP 202/PENDING contract, then poll the persisted run until it reaches a terminal status."
source: cookbook/05_agent_os/04_run_lifecycle/background_run.py
---

```python background_run.py
"""
...
<Steps>
  ...
  <Step title="Run the example">
    Save the code above as `background_run.py`, then run:
    ```bash
    python background_run.py
    ```
  </Step>
...
Full source: [cookbook/05_agent_os/04_run_lifecycle/background_run.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/05_agent_os/04_run_lifecycle/background_run.py)
