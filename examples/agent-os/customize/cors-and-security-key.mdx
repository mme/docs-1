---
title: "Configure CORS and a shared AgentOS security key"
description: "``cors_allowed_origins`` replaces AgentOS's default allowed origins."
source: cookbook/05_agent_os/06_customize/cors_and_security_key.py
---

``cors_allowed_origins`` replaces AgentOS's default allowed origins. ``AgnoAPISettings(os_security_key=...)`` protects the REST surface with a Bearer token. The client verifies an unauthenticated rejection, an authenticated discovery request, and an allowed CORS preflight.

```python cors_and_security_key.py
"""
...
<Steps>
  ...
  <Step title="Run the example">
    Save the code above as `cors_and_security_key.py`, then run:
    ```bash
    python cors_and_security_key.py
    ```
  </Step>
...
Full source: [cookbook/05_agent_os/06_customize/cors_and_security_key.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/05_agent_os/06_customize/cors_and_security_key.py)
