---
title: "Filter Traces Through AgentOS"
description: "Build a composable FilterExpr, apply it directly to the tracing database, and send the same expression to POST /traces/search."
source: cookbook/05_agent_os/13_observability/filtering.py
---

Build a composable FilterExpr, apply it directly to the tracing database, and send the same expression to POST /traces/search. GET /traces/filter-schema provides the fields and operators a client can expose.
...
Full source: [cookbook/05_agent_os/13_observability/filtering.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/05_agent_os/13_observability/filtering.py)
