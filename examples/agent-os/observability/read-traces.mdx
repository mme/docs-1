---
title: "Read Trace Trees Through AgentOS"
description: "Generate one synchronous and one asynchronous agent run, list their traces through GET /traces, then fetch GET /traces/{trace_id} and print each nested span tree."
source: cookbook/05_agent_os/13_observability/read_traces.py
---

```python read_traces.py
"""
...
<Steps>
  ...
  <Step title="Run the example">
    Save the code above as `read_traces.py`, then run:
    ```bash
    python read_traces.py
    ```
  </Step>
...
Full source: [cookbook/05_agent_os/13_observability/read_traces.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/05_agent_os/13_observability/read_traces.py)
