---
title: "Serve an Agent over A2A"
description: "Expose one persistent Agent through AgentOS's built-in A2A interface."
source: cookbook/05_agent_os/15_a2a/basic.py
---

Expose one persistent Agent through AgentOS's built-in A2A interface. The `a2a_interface=True` shorthand exposes every registered entity; use an explicit `interfaces=[A2A(agents=[...])]` when you need to select which entities are exposed or customize interface tags.
...
Full source: [cookbook/05_agent_os/15_a2a/basic.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/05_agent_os/15_a2a/basic.py)
