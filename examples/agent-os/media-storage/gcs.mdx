...
Demonstrates keeping media bytes out of the database. AgentOS sends attached and generated files to Google Cloud Storage and persists only a MediaReference.
...
<Steps>
  ...
  <Step title="Install dependencies">
    ```bash
    uv pip install -U "agno[gcs,os]" openai python-docx python-dotenv reportlab
    ```
  </Step>
  ...
  <Step title="Export environment variables">
    <CodeGroup>
    ```bash Mac/Linux
    export AGNO_FILE_OUTPUT_GCS_BUCKET="your_agno_file_output_gcs_bucket_here"
    ...
    ```bash Windows
    $Env:AGNO_FILE_OUTPUT_GCS_BUCKET="your_agno_file_output_gcs_bucket_here"
...
Full source: [cookbook/05_agent_os/02_databases/gcs_media_storage.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/05_agent_os/02_databases/gcs_media_storage.py)
