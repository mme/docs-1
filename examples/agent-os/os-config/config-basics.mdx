---
title: "Configure AgentOS in Python"
description: "Attach UI metadata and database-domain labels to an AgentOSConfig, then fetch GET /config to inspect the rendered control-plane configuration."
source: cookbook/05_agent_os/08_os_config/config_basics.py
---

```python config_basics.py
"""
...
<Steps>
  ...
  <Step title="Run the example">
    Save the code above as `config_basics.py`, then run:
    ```bash
    python config_basics.py
    ```
  </Step>
...
Full source: [cookbook/05_agent_os/08_os_config/config_basics.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/05_agent_os/08_os_config/config_basics.py)
