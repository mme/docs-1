---
title: "Workflow Media Storage (S3)"
description: "Demonstrates media offload across a workflow."
source: cookbook/06_storage/10_media_storage_workflow.py
---

Step inputs are rehydrated before each step runs, so a step downstream of an offloaded one receives the bytes rather than an empty pointer.

```python media_storage_workflow.py
"""
...
<Steps>
  ...
  <Step title="Install dependencies">
    ```bash
    uv pip install -U "agno[s3]" fastapi openai sqlalchemy
    ```
  </Step>
...
<Step title="Configure AWS credentials">
  ...
  <Step title="Run the example">
    Save the code above as `media_storage_workflow.py`, then run:
    ```bash
    python media_storage_workflow.py
    ```
  </Step>
...
Full source: [cookbook/06_storage/10_media_storage_workflow.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/06_storage/10_media_storage_workflow.py)
