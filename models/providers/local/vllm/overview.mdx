...
<Warning>
The command below requires a supported Linux GPU environment. Choose the installation path for your hardware and operating system in the [vLLM GPU installation guide](https://docs.vllm.ai/en/stable/getting_started/installation/gpu/) before running it.
</Warning>

```bash install vLLM
uv pip install -U agno openai vllm --torch-backend=auto
```
