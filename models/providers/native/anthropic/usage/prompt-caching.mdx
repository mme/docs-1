...
Blocks are appended after the agent-built system message in the Anthropic `system` array. Pass a zero-argument callable to evaluate dynamic blocks on every request without reinstantiating the model. See [Prompt Caching With a Dynamic Block](/examples/models/anthropic/prompt-caching-with-dynamic-block).
