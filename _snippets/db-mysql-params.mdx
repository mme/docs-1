...
| `runs_table` | `Optional[str]` | `None` | Table for session runs. Uses `"agno_runs"` with the default session table, or `<session_table>_runs` when `session_table` is customized. |
