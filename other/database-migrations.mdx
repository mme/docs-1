...
You can find the supporting scripts to handle your migration needs in the [`libs/agno/migrations`](https://github.com/agno-agi/agno/tree/v3.0.4/libs/agno/migrations) directory.
...
The supported table types are: `memories`, `sessions`, `metrics`, `evals`, `knowledge`, `approvals`, `components`, `schedules`, `schedule_runs`, and `learnings`.
...
<Warning>
  Some migrations are irreversible. In v3.0, the `learnings` entity-memory re-key cannot be moved back to the shared pre-v3 key. `MigrationManager.down()` leaves that table unchanged. Back up the database before downgrading.
</Warning>
