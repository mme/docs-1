...
When `user_isolation=True`, non-admin callers are scoped to the `user_id` from the JWT `sub` claim across sessions, memories, traces, approvals, and other user-owned AgentOS resources. Callers holding `admin_scope` bypass isolation. See [Per-User Data Isolation](/agent-os/security/authorization/user-isolation) for the full behavior.
