...
<Note>
**Agent instance and run state**: Agents keep working session state in the run context and database. The Agent instance retains configuration and a default session identity:
...
- When no `session_id` is provided, the first run generates an ID and stores it on `agent.session_id`. Later runs on the same Agent reuse that ID unless you pass a different one.
</Note>
...
- [State cookbook](https://github.com/agno-agi/agno/tree/main/cookbook/02_agents/05_state_and_session)
