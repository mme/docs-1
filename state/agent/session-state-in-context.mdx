...
description: "Persist each user's name and age across runs in separate session state."
mode: wide
---

Use session state to carry user context across runs in one session. Each session stores its own state, so the two users remain isolated.
